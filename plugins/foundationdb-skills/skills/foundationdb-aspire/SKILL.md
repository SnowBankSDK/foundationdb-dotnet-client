...
description: How to run a FoundationDB cluster and connect to it from .NET — the parts that come *before* you have an IFdbDatabase/IFdbDatabaseProvider (the keys/transactions/layers skills assume you already have one). Covers the two ways to get a provider (plain DI services.AddFoundationDb(apiVersion, …) / FdbDatabaseProvider.Create, vs the Aspire integration), the .NET Aspire AppHost integration (FoundationDB.Aspire.Hosting: builder.AddFoundationDb("fdb", apiVersion, root, clusterVersion) starts a Docker container; AddFoundationDbCluster connects to an existing one), the Aspire client integration (FoundationDB.Aspire: builder.AddFoundationDb("fdb", settings, options) reads the injected connection), wiring them with WithReference/WaitFor, launching with the `aspire` CLI (aspire run) vs plain dotnet run + launchSettings, the native client (libfdb_c via UseNativeClient, which platforms FoundationDB.Client.Native ships, and the macOS system-library fallback), and the client⇄cluster version-compatibility rule. Use whenever code opens/connects to a cluster, registers AddFoundationDb (in an AppHost or an app), runs an Aspire host, or hits libfdb_c load failures / transactions that hang on a fresh cluster.
---
