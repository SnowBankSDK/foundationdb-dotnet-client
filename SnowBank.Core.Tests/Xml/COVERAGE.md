...
| no `xmlns`, no prefixes, no `z:Id`/`z:Ref` | every byte-equality fact (the oracle wire is prefix-free; any emitted prefix would fail equality) |

## 2. Member ordering

| Trait | Covering tests |
|---|---|
| default ordinal-alphabetical on the wire name | `DcsWireFidelityFacts.Test_Member_Order_Default_Is_Ordinal_Alphabetical` |
...
and `[@type = ...]` discriminator reads) transforming BOTH the CrystalXml wire and the live-DCS
wire into identical HTML, for a populated and an all-null account, plus direct whole-document
wire equality.
...
5. COVERED (this ledger's own probe) - `List<object>` as a collection member (matrix:
   ...
   `AsObjectInt`), not the undeclared-runtime-type refusal of gap 1 (which fires only when the
...
6. OUT OF SCOPE, MEASURED ABSENT - `[DataContract(IsReference=true)]` / `z:Id`/`z:Ref` object
   tracking: exactly one declaration in the measured solution, zero wire occurrences, no support
   and no test.
