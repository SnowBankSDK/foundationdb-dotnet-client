...
Sans Aspire, vous démarrez un container, vous l'attendez, vous configurez la database et vous copiez
...
bonne santé, donc un service ne démarre pas contre une database qui ne peut pas encore répondre.

## Un cluster neuf se provisionne lui-même

Une database FoundationDB sur un volume de stockage tout neuf n'est pas utilisable tant qu'elle n'a
pas été configurée une fois. Lancez `status` dessus et elle signale « The database is unavailable » ;
...
qui attend le cluster jusqu'à ce que la database réponde. Une database déjà configurée est laissée
intacte, donc un redémarrage avec un volume persistant saute l'étape et journalise que la database est
déjà configurée. Le travail est idempotent et sûr quand deux initiateurs démarrent en même temps ;
deux AppHosts contre un même volume neuf convergent vers une seule database configurée, pas deux
databases en conflit.

Deux propriétés rendent ce comportement sûr à laisser activé par défaut. L'attente est **bornée**, pas
infinie : si la database ne devient pas disponible à temps, l'AppHost échoue et nomme la recette
manuelle `fdbcli --exec "configure new single ssd"`, plutôt que de rester bloqué en silence. Et le
chemin nominal **journalise** qu'il a configuré la database, donc un premier démarrage n'est jamais
...
fraîchement créé provisionne lui-même sa database au premier démarrage, donc une suite de tests n'a
...
supposent que vous avez déjà. À partir du provider, vous ouvrez la database et vous lisez et écrivez
