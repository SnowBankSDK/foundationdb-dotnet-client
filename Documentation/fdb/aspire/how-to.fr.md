...
Une exécution ultérieure retrouve le même volume, donc la base de données est déjà configurée et l'étape de
...
L'intégration configure une base de données neuve au premier démarrage. Pour gérer ça vous-même,
...
Un volume neuf n'a alors pas de base de données, et tout service qui l'ouvre attend sans erreur jusqu'à ce
...
L'AppHost et le *test harness* configurent tous deux une base de données neuve à travers une seule primitive,
...
`fdbcli`, un *timeout*, et retourne une fois la base de données disponible :
...
Elle est idempotente : une base de données déjà configurée est laissée intacte. Si la base de données n'est pas
